...
Note that if db-only mode enabled, re-trying procedure works for notification because of no existed recorded file on storage. (see B<output-storage>, B<--notify-record>)
