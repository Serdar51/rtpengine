...
static void amr_encoder_mode_change(encoder_t *enc) {
	...
	ilog(LOG_WARNING, "[SERDAR][amr_encoder_mode_change][3][%d][%llu] --- CMR : %u", amr.cmr_out_seq, (unsigned long long)amr.pkt_seq, cmr);
	if (cmr == 0xffff)
		cmr = amr_encoder_find_next_mode(enc);
	if (cmr >= AMR_FT_TYPES)
		return;
	ilog(LOG_WARNING, "[SERDAR][amr_encoder_mode_change][4][%d][%llu] --- CMR Next : %u", amr.cmr_out_seq, (unsigned long long)amr.pkt_seq, cmr);
	...
	ilog(LOG_WARNING, "[SERDAR][amr_encoder_mode_change][5][%d][%llu] --- Req Bitrate : %d", amr.cmr_out_seq, (unsigned long long)amr.pkt_seq, req_br);
	if (!req_br)
		return;
	int cmr_done = 1;
	ilog(LOG_WARNING, "[SERDAR][amr_encoder_mode_change][6][%d][%llu]", amr.cmr_out_seq, (unsigned long long)amr.pkt_seq);
	if (enc->format_options.amr.mode_change_neighbor) {
		...
		int neigh_br = req_br;
		int cmr_br = req_br;
		ilog(LOG_WARNING, "[SERDAR][amr_encoder_mode_change][6][1][%d][%llu] --- Request Bit Rate : %d, Current Bit Rate : %d, Neigbour Bit Rate : %d, Neighbour Bit Rate : %d, Cmr Bit Rate : %d", amr.cmr_out_seq, (unsigned long long)amr.pkt_seq, req_br, cur_br, neigh_br, cmr_br);
		while (1) {
			...
			neigh_br = cmr_br;
			ilog(LOG_WARNING, "[SERDAR][amr_encoder_mode_change][6][X][%d][%llu] --- Request Bit Rate : %d, Current Bit Rate : %d, Neigbour Bit Rate : %d, Neighbour Bit Rate : %d, Cmr Bit Rate : %d", amr.cmr_out_seq, (unsigned long long)amr.pkt_seq, req_br, cur_br, neigh_br, cmr_br);
		}
		// did we finish stepping or is there more to go?
		ilog(LOG_WARNING, "[SERDAR][amr_encoder_mode_change][6][2][%d][%llu] --- Request Bit Rate : %d, Current Bit Rate : %d, Neigbour Bit Rate : %d, Neighbour Bit Rate : %d, Cmr Bit Rate : %d", amr.cmr_out_seq, (unsigned long long)amr.pkt_seq, req_br, cur_br, neigh_br, cmr_br);
		if (neigh_br != req_br)
			cmr_done = 0;
		req_br = neigh_br; // set to this
	}
	
	ilog(LOG_WARNING, "[SERDAR][amr_encoder_mode_change][7][%d][%llu] --- Request Bit Rate : %d, Cmr Done : %d", amr.cmr_out_seq, (unsigned long long)amr.pkt_seq, req_br, cmr_done);
	enc->u.avc.avcctx->bit_rate = req_br;
	if (cmr_done)
		enc->u.avc.u.amr.cmr_in_ts = enc->callback.amr.cmr_in_ts;
}
static void amr_encoder_got_packet(encoder_t *enc) {
	ilog(LOG_WARNING, "[SERDAR][amr_encoder_got_packet][1][%d][%llu] --- amr_encoder_got_packet worked! ", amr.cmr_out_seq, (unsigned long long)amr.pkt_seq);
...
static int packetizer_amr(AVPacket *pkt, GString *buf, str *output, encoder_t *enc) {
	ilog(LOG_WARNING, "[SERDAR][packetizer_amr][1][%d][%llu] --- packetizer_amr worked! ", amr.cmr_out_seq, (unsigned long long)amr.pkt_seq);
	assert(pkt->size >= 1);
